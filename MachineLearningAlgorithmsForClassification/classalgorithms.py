...
class NeuralNet(Classifier):
    ...
    def learn(self, Xtrain, ytrain):
        ...
        self.wi=np.random.randint(1,size=(Xtrain.shape[1], self.params['nh']))
...
class RadialBasisNetwork(Classifier):
    def __init__( self, parameters={} ):
        # Default: no regularization
        self.params = {'Beta': 1, 'regularizer': 'None'}
        self.numberC=400
        self.reset(parameters)
        self.center=[]
    ...
    def learn(self, Xtrain, ytrain):
        #Taking centroids as Random values from Training Sample 	
        XtrainC=np.zeros((self.numberC,Xtrain.shape[1]))
        for i in range(self.numberC):
            for j in range(Xtrain.shape[1]):
                XtrainC[i][j]=np.random.choice(Xtrain[i])
        Beta = self.params['Beta']    				
        phi=np.zeros((Xtrain.shape[0],self.numberC))
        #Radial Basis Function Transformation		
        for k in range(Xtrain.shape[0]):
            for j in range(self.numberC):
                phi[k][j]=np.exp(np.linalg.norm(np.dot((Xtrain[k]-XtrainC[j]).T ,(Xtrain[k]-XtrainC[j]))*-1*Beta))
        self.center=XtrainC                    
        self.weights=np.dot(np.dot(np.linalg.pinv(np.dot(phi.T,phi)),phi.T),ytrain)
        p=utils.sigmoid(np.dot(phi,self.weights))
        Err=np.linalg.norm(np.subtract(ytrain,p))
        while(True):    		
            P=np.diag(p)
            self.weights=self.weights+np.dot(np.linalg.pinv(np.dot(np.dot(np.dot(phi.T,P),(np.identity(P.shape[0])-p)),phi)),np.dot(phi.T,np.subtract(ytrain,p)))		
            p=utils.sigmoid(np.dot(phi,self.weights))
            Error=np.linalg.norm(np.subtract(ytrain,p))
            if(abs(Err-Error)<0.01):
                break
    ...
    def predict(self, Xtest):
        Beta = self.params['Beta']		
        phi1=np.zeros((Xtest.shape[0],self.numberC))
        #Radial Basis Function Transformation			
        for k in range(Xtest.shape[0]):
            for j in range(self.numberC):
                phi1[k][j]=np.exp(np.linalg.norm(np.dot((Xtest[k]-self.center[j]).T ,(Xtest[k]-self.center[j]))*-1*Beta))
        ytest = utils.sigmoid(np.dot(phi1, self.weights))
        ytest[ytest >=0.5] = 1     
        ytest[ytest < 0.5] = 0    
        return ytest           
